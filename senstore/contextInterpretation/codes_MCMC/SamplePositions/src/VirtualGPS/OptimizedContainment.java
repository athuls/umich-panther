...
public class OptimizedContainment{ 
	...
	public OptimizedContainment(Hashtable<Integer, List<Nodes> > nodesList, double[] inspectorPos, double[] nearPlane, double[] farPlane, double viewAngle, int truePos, int observedPos, int sampleNum, int runNum) {
		...
		m_outputElementFile = Config.getUniqueBoundedElements().concat(Integer.toString(runNum)+"/position"+Integer.toString(truePos)+"/observedPos"+Integer.toString(observedPos)+"/"+Integer.toString(sampleNum));
	...
	public OptimizedContainment(Matrix inspectorPos, Matrix farPlane, Matrix nearPlane, double viewAngle, int runNum, int truePosNum, int observedPosNum) {
		...
		m_distanceToFarPoint=m_MI.minus(m_farPlanePoint).normF();
		m_planeNormal = (m_farPlanePoint.minus(m_MI)).times((double)((double)1/(double)m_distanceToFarPoint));
		m_outputElementFile = Config.getUniqueBoundedElements().concat(Integer.toString(runNum)+"/position"+Integer.toString(truePosNum)+"/"+Integer.toString(observedPosNum));
	}
	...
	public void getContainedNodesInMeanCone() throws IOException
	{
		...
		BufferedWriter out_unique_elements=new BufferedWriter(new FileWriter(m_outputElementFile));
                ...
                if(m_trueInspectorZPos > (Config.getDeckZPosition()))
	...
	public Hashtable<Integer,List<Nodes> > getContainedNodes()
	{
		int element_id;
                ...
                Hashtable<Integer,List<Nodes> > contextualElements=new Hashtable<Integer,List<Nodes> >();
                ...
                for(int i=0;i<m_nodesList.size();i++)
                {
                                ...
                                if(checkContainment())
                                {      
					//If the element is not in the structure, create a new list and add it as value for the element 
					if(!contextualElements.containsKey(element_id))
						contextualElements.put(element_id, new ArrayList<Nodes> ());
					(contextualElements.get(element_id)).add(m_nodesList.get(i));
				}
                }
