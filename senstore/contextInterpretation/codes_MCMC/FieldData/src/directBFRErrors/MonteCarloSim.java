...
import directBFRErrors.OptimizedContainment;
...
public class MonteCarloSim 
{
	...
	public MonteCarloSim(double[] observedInspectorPos, double[] observedInspectorOrient, int runs, int truePositionNum, int observedPosNum, int simSampleNum, double npdDist, double fpdDist, double viewAngle) throws IOException
	{
		m_observedInspectorPos = observedInspectorPos;
		m_observedInspectorOrient=observedInspectorOrient;
		m_observedPositionNum=observedPosNum;	
	...
	public void simulatePositions() throws IOException
	{
		...
		Matrix[] npdPoints = new Matrix[m_simSampleNum];
		...
		while(samples<m_simSampleNum)
		{
			...
			npdPoints[samples] = new Matrix(new double[][]{npdInBFR});
		...
		try{
                	OptimizedContainment boundingBox=new OptimizedContainment(inspectorPositions[0], fpdPoints[0], npdPoints[0], m_viewAngle, m_runNum, m_truePositionNum, m_observedPositionNum);
	                boundingBox.getContainedNodes();
		}
		catch(Exception e){
