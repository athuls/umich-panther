...
public class PositionContext	
{
	...
	public PositionContext(double[] trueInspectorPos, double[] trueInspectorOrient, int run, int position) throws IOException
	{
		//Get the inspectors position and orientation
		m_trueInspectorPos = trueInspectorPos;
		m_trueInspectorOrient = trueInspectorOrient; 
		...
		m_numObservedSamples = ConfigErrorAnalysis.getNumObsPos();
		m_numSimulatedSamples = ConfigErrorAnalysis.getNumSimPos();
		System.out.println("I am in position context");
		startProcessing();
	}
	...
	private void startProcessing() throws IOException
	{
		...
		while(countObserved < m_numObservedSamples)
		{
			...
			MonteCarloSim simObject=new MonteCarloSim(m_trueInspectorPos, m_trueInspectorOrient, m_runNum, m_truePositionNum, countObserved, m_numSimulatedSamples, npdDistance, fpdDistance, viewAngle);		
