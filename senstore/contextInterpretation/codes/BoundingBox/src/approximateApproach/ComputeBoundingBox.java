...
public class ComputeBoundingBox implements ComputeBoundingBoxInterface{
	...
	private	 Matrix planeNormal=new Matrix(1,3);
    private Matrix M_I, M_P, M_F1, M_F2;
...
*/
	...
	public void computePlaneNormal() {
		for(int i=0;i<3;i++) planeNormal.set(0,i,((M_F2.minus(M_I).get(0,i))/((M_F2.minus(M_I)).normF())));
	...
	public boolean checkContainment() {
		...
		//check for containment of query_point in the bounding box
			...
			if((M_P.minus(M_I).get(0,0)*planeNormal.get(0,0)+M_P.minus(M_I).get(0,1)*planeNormal.get(0,1)+M_P.minus(M_I).get(0,2)*planeNormal.get(0,2)) > 0)
			{
				...
				if(phiAngle <= m_viewAngle) {
					double distance_to_inspector = M_I.minus(M_P).normF()*Math.cos(phiAngle); 
		 			if(M_I.minus(M_F1).normF()<=distance_to_inspector && distance_to_inspector<=M_I.minus(M_F2).normF())
						return true;
					else
						return false;
				}
